...
void main (void)
{
	...
	while(1)
	{
		while(!transmit)			// while trasmitter not transmitting, check until turned on
		{
			LEDs(0, 0, 0, 0);
			if(!P2_2)
			{
				Timer4ms(50);
				while(!P2_2);		// wait for button unpress
				transmit = 1;		// transmit flag on
				Timer4ms(50);
			}
			printf("\rOFF");
		...
		while(transmit)
		{
			...
			if(!P2_2)				// check transmit button press
			{
				Timer4ms(50);
				while(!P2_2);		// wait for button unpress
				transmit = 0;		// transmit flag off
				Timer4ms(50);
			}
