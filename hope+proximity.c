...
char _c51_external_startup (void)
{
	...
	P1MDOUT = 0b1111_0000;    //p1.4-1.7 output pins 
	P2MDOUT |= 0b1111_0100;   // P2.0-2.1 ADC input pins, P2.4-2.7 output pins
...
void Timer2_ISR (void) interrupt 5
{
	...
	if(motor==0){
		...
		num=10;
		SERVO180=pwm_count>pwmwidth?0:1;
	}	
...
void main (void)
{
	...
	while(1)
	{
		...
		if (command==AUTO){
			...
			if (d1 > 0.35)		// if d1 is further back than desired d including threshold
			{
				...
				//buffer zone of 
			else if (d1 < 0.20){		// if d1 is closer than desired d + threshold
			...
			//Timer4ms(100);  // Wait 100ms before next round of measurements.
